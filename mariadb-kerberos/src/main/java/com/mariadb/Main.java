...
public class Main {
  ...
  private static class MariaDBJDBCConfiguration extends Configuration {
    private Configuration parent;
    Map<String, String> options = new HashMap<>();
    AppConfigurationEntry[] entry;

    public MariaDBJDBCConfiguration(Configuration parent, String keytab, String principal) {
      this.parent = parent;
    ...
    public AppConfigurationEntry[] getAppConfigurationEntry(String name) {
      if (name.equals("Krb5ConnectorContext")) {
        return entry;
      } else {
  ...
  public static void main(String[] args) throws SQLException, IOException {
    ...
    LOGGER.info("mariadb-kerberos application started");

    if (args.length != 3) {
      System.out.println("Usage: mariadb-kerberos <keytab> <principal> <url>");
      System.out.println("Example: mariadb-kerberos share/mariadb.keytab mariadb/example.com@EXAMPLE.COM \"jdbc:mariadb://example.com/mysql?user=mariadb/example.com@EXAMPLE.COM");
      System.exit(1);
    }
    ...
    MariaDBJDBCConfiguration config = new MariaDBJDBCConfiguration(Configuration.getConfiguration(), args[0], args[1]);
